...
import { Wallet, PieChart, CreditCard, Send, Plus, Bell, Home, MoreHorizontal, ArrowLeftRight } from "lucide-react";
const TRANSACTIONS = [
  { id: 1, name: "Apple Music", type: "Subscription", amount: -10.99, icon: "\u{1F3B5}", date: "Today, 10:00 AM", color: "bg-rose-100 dark:bg-rose-900/30 text-rose-500" },
  { id: 2, name: "Salary", type: "Income", amount: 4500, icon: "\u{1F4BC}", date: "Yesterday", color: "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-500" },
  { id: 3, name: "Coffee Shop", type: "Food", amount: -4.5, icon: "\u2615", date: "Yesterday", color: "bg-amber-100 dark:bg-amber-900/30 text-amber-500" },
  { id: 4, name: "Uber Ride", type: "Transport", amount: -12.4, icon: "\u{1F697}", date: "Oct 24", color: "bg-blue-100 dark:bg-blue-900/30 text-blue-500" },
  { id: 5, name: "Grocery Store", type: "Shopping", amount: -85.2, icon: "\u{1F6D2}", date: "Oct 22", color: "bg-purple-100 dark:bg-purple-900/30 text-purple-500" }
];
const CARDS = [
...
function FintechShowcase() {
  const [activeTab, setActiveTab] = useState("home");
  const [activeCard, setActiveCard] = useState(0);
  const renderHome = () => <div className="flex-1 overflow-y-auto scrollbar-hide pb-24">
      {
    /* Header */
  }
  ...
  >
          <div className={`absolute inset-0 bg-gradient-to-br ${CARDS[activeCard].gradient} transition-all duration-500`} />
    ...
    /* Pagination Dots */
        ...
        <div className="flex justify-center gap-1.5 mt-4">
          {CARDS.map((_, i) => <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${i === activeCard ? "w-4 bg-gray-900 dark:bg-white" : "w-1.5 bg-gray-300 dark:bg-gray-700"}`} />)}
        </div>
      </div>
  ...
  ].map((action, i) => <div key={i} className="flex flex-col items-center gap-2 cursor-pointer group">
    ...
    /* Transactions */
      ...
      <div className="px-6 animate-in fade-in slide-in-from-bottom-12 duration-500 delay-300 fill-mode-both">
        ...
        <div className="space-y-4">
          {TRANSACTIONS.map((tx) => <div key={tx.id} className="flex items-center gap-4 group cursor-pointer">
  ...
  return <div className="w-full h-full bg-slate-50 dark:bg-black relative flex flex-col overflow-hidden font-sans">
      {activeTab === "home" && renderHome()}
      {activeTab !== "home" && <div className="flex-1 flex items-center justify-center text-gray-400 dark:text-gray-600">
          Coming soon
        </div>}
  ...
  >
          ...
          {activeTab === "home" && <div className="w-1 h-1 rounded-full bg-black dark:bg-white mt-0.5" />}
        </button>
        <button
  ...
  >
          ...
          {activeTab === "stats" && <div className="w-1 h-1 rounded-full bg-black dark:bg-white mt-0.5" />}
        </button>
  ...
  >
          ...
          {activeTab === "wallet" && <div className="w-1 h-1 rounded-full bg-black dark:bg-white mt-0.5" />}
        </button>
        <button
  ...
  >
          ...
          {activeTab === "profile" && <div className="w-1 h-1 rounded-full bg-black dark:bg-white mt-0.5" />}
        </button>
      </div>
    </div>;
}
export {
  FintechShowcase as default
};
