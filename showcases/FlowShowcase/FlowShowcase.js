...
function FlowShowcase() {
  ...
  return <div className="w-full max-w-7xl mx-auto h-[calc(100vh-64px)] p-4 md:p-8 flex flex-col">
  ...
  >
          {isDetecting ? <IoRefreshOutline className="animate-spin w-5 h-5" /> : <IoAnalyticsOutline className="w-5 h-5" />}
          Auto Detect Flow
        </button>
      </div>
      ...
      <div className="flex-1 bg-white dark:bg-[#1C1C1E] border border-gray-200 dark:border-gray-800 rounded-3xl overflow-hidden relative shadow-inner p-8">
        ...
        {screens.length === 0 ? <div className="flex flex-col items-center justify-center h-full text-center opacity-60">
          ...
          </div> : <div className="w-full h-full overflow-auto custom-scrollbar relative flex items-center justify-start p-10 min-w-max">
            {screens.map((screen, index) => {
    const hasNext = index < screens.length - 1;
    const edge = edges.find((e) => e.from === screen);
    return <div key={screen} className="flex items-center">
                  ...
                  {hasNext && <div className="w-32 flex flex-col items-center justify-center relative px-2">
                       {edge ? <>
