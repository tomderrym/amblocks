...
const PhoneFrame: React.FC<{children: React.ReactNode}> = ({ children }) => (
  <div className="w-[320px] h-[640px] bg-white dark:bg-black rounded-[40px] border-[12px] border-black shadow-xl overflow-hidden relative flex flex-col shrink-0">
    {children}
  </div>
);

export default function BreathingShowcase() {
    ...
    if (isActive) {
      timerRef.current = setInterval(() => {
        setTimeLeft((prevTime) => {
          if (prevTime <= 1) {
            // Move to the next phase of the breathing cycle
            const nextStepIndex = (stepIndex + 1) % currentTech.steps.length;
            setStepIndex(nextStepIndex);
            
            // If we looped back to 0, a full cycle was completed
            if (nextStepIndex === 0) {
  ...
  const selectTechnique = (tech: any) => {
    setCurrentTech(tech);
    setStepIndex(0);
  ...
  // Dynamic scaling for the breathing circle based on the current phase
  let scale = 1;
  let transitionDuration = "1000ms"; // default 1s transition

  if (isActive) {
    if (currentStep.label === 'Inhale') {
      // Inhale expands the circle smoothly over the duration
      scale = 1.6;
      transitionDuration = `${currentStep.duration}s`;
    } else if (currentStep.label === 'Exhale') {
      // Exhale shrinks the circle back to normal
      scale = 1;
      transitionDuration = `${currentStep.duration}s`;
    } else if (currentStep.label === 'Hold') {
      // Hold maintains the previous scale
  ...
  return (
    <div className="w-full h-full min-h-screen bg-slate-900 flex flex-col items-center justify-between p-6 font-sans text-slate-100 overflow-hidden relative">
      <div 
      ...
      <div className="flex-1 flex flex-col items-center justify-center w-full max-w-md relative z-10 my-8">
            ...
            <div className="relative flex items-center justify-center w-[200px] h-[200px]">
              ...
              >
                <div className="flex flex-col items-center">
                  ...
                  {isActive && (
                    <span className="text-white/80 text-xs font-medium tracking-widest uppercase mt-1">
                      sec
                    </span>
                  )}
                </div>
          ...
          <div className="w-full max-w-md pb-4 flex flex-col gap-6 items-center z-10 relative">
            ...
            <div className="flex items-center justify-center gap-4 w-full">
              ...
              <button 
                onClick={toggleApp}
                className={`w-16 h-16 rounded-full flex items-center justify-center transition-all duration-300 shadow-2xl hover:scale-105 active:scale-95 border-none outline-none ${
                  isActive 
                    ? 'bg-slate-100 text-slate-900' 
                    : `bg-gradient-to-tr ${currentTech.color} text-white`
                }`}
              >
                {isActive ? (
          ...
          {showSettings && (
            <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-end justify-center">
                ...
                <div className="space-y-3">
                  {TECHNIQUES.map((tech) => (
                    <button
                      ...
                      className={`w-full p-3 rounded-2xl flex items-center justify-between transition-all border outline-none ${
                        currentTech.id === tech.id 
                          ? `bg-slate-700/50 border-slate-500` 
                          : 'bg-slate-800/50 border-transparent hover:border-slate-600 hover:bg-slate-700/30'
                      }`}
                    >
