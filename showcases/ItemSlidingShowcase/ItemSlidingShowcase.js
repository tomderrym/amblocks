...
import { IonAvatar, IonCard, IonCardContent, IonCardHeader, IonCardSubtitle, IonCardTitle, IonLabel } from "../../ionic-replica";
const SwipeableItem = ({ children, options }) => {
  return <div className="relative w-full border-b border-[rgba(60,60,67,0.29)] dark:border-[#38383A] last:border-b-0">
...
const SwipeOption = ({ children, color = "primary", onClick }) => {
  const cMap = { primary: "bg-[#3880FF]", danger: "bg-[#eb445a]", success: "bg-[#2dd36f]", warning: "bg-[#ffc409]" };
  return <button onClick={onClick} className={`px-5 py-2 text-white font-medium flex flex-col items-center justify-center h-full active:brightness-90 transition-all ${cMap[color] || cMap.primary}`}>
      {children}
    </button>;
};
function ItemSlidingShowcase() {
  return <>
      <IonCard>
        <IonCardHeader>
...
export {
  ItemSlidingShowcase as default
};
