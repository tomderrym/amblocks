             ...
             <div className="w-full max-w-sm">
                <IonItem lines={variant === 'bordered' || variant === 'none' ? 'none' : 'inset'}>
                  <IonInput 
                    ...
                    leftIcon={showIcon ? <Search className="w-5 h-5"/> : undefined}
          ...
          <IonList className="bg-transparent">
             <IonItem>
                <div className="w-full py-2">
                   ...
                   <div className="flex gap-2">
                      {['default', 'bordered', 'none'].map(v => (
                         <button key={v} onClick={() => setVariant(v)} className={`px-4 py-1.5 rounded-full text-xs font-medium border ${variant === v ? 'bg-blue-500 text-white border-blue-500' : 'bg-transparent border-gray-300 dark:border-gray-700 text-gray-600 dark:text-gray-300'}`}>
                            {v}
                         </button>
                      ))}
                   </div>
             ...
             <IonItem>
                <div className="w-full py-2">
                   ...
                   <div className="flex gap-2">
                      {['stacked', 'floating', 'start'].map(l => (
                         <button key={l} onClick={() => setLabelPlacement(l)} className={`px-4 py-1.5 rounded-full text-xs font-medium border ${labelPlacement === l ? 'bg-blue-500 text-white border-blue-500' : 'bg-transparent border-gray-300 dark:border-gray-700 text-gray-600 dark:text-gray-300'}`}>
                            {l}
                         </button>
                      ))}
                   </div>
