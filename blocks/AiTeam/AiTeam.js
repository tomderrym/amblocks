function aiteam() {
  return <div className="w-full bg-white dark:bg-[#1A1A1A] border border-gray-200 dark:border-[#2A2A2A] rounded-xl overflow-hidden p-6 mb-2 shadow-sm text-center" style={{ pointerEvents }}>
                    ...
                    <div className="flex justify-center -space-x-2 mb-4">
                       {[1, 2, 3, 4].map((i) => <div key={i} className={`w-10 h-10 rounded-full border-2 border-white dark:border-[#1A1A1A] flex items-center justify-center text-xs shadow-sm bg-gray-100 dark:bg-gray-800 z-${10 - i} relative`}>
                           {["\u{1F469}", "\u{1F468}", "\u{1F9D1}", "\u{1F471}"][i - 1]}
                         </div>)}
