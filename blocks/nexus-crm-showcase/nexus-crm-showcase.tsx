...
const LayoutContainer = ({ children, style }: any) => (
  <div style={style} className="nexus-layout-container">
    {children}
  </div>
);

const Container = ({ children, style }: any) => (
  <div style={style} className="nexus-container">
    {children}
  </div>
);

const Header = ({ text, style, binding, appData }: any) => {
  const value = binding && appData ? appData[binding] : '';
  return <h1 style={{ margin: 0, ...style }} className="nexus-header">{text || ''}{value}</h1>;
};

const Text = ({ text, style }: any) => (
  <p style={{ margin: 0, ...style }} className="nexus-text">{text}</p>
);

const Input = ({ placeholder, binding, style, setVariable, appData }: any) => {
  return (
    <input
      type="text"
...
const Button = ({ text, style, action, executeAction }: any) => {
  return (
    <button
      style={style}
...
const Card = ({ children, style }: any) => (
  <div style={style} className="nexus-card">
    {children}
  </div>
);
...
// 2. THE GATEKEEPER
const shouldRender = (node: any, appData: any) => {
...
// 3. RECURSIVE ENGINE
const RenderNode = ({ node, appData, setVariable, executeAction }: any) => {
...
export default function NexusCrmShowcase() {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-800">
      <div className="flex-1 overflow-hidden relative">
        {crmSchema.uiSchema.map((node) => (
          <RenderNode
            key={node.id}
            node={node}
            appData={appData}
