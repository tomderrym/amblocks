    ...
    <div className="w-full bg-white dark:bg-[#1A1A1A] border border-gray-200 dark:border-[#2A2A2A] rounded-xl overflow-hidden p-4 mb-2 shadow-sm" style={{ pointerEvents }}>
                    ...
                    <div className="grid grid-cols-7 gap-1 text-center">
                       ...
                       {Array.from({length: 30}).map((_, i) => (
                         <button key={i} className={`h-6 flex items-center justify-center rounded text-xs transition-colors hover:bg-gray-100 dark:hover:bg-gray-800 ${i === 14 ? 'bg-blue-500 hover:bg-blue-600 text-white font-bold' : 'text-gray-700 dark:text-gray-300'}`}>
                           {i + 1}
                         </button>
                       ))}
                    </div>
