...
import { IonCard, IonCardContent, IonCardHeader, IonCardTitle } from '../../ionic-replica';
import { ShoppingBag, Search, Heart, User, ChevronLeft, Star, Menu, Plus, Minus, Home, Grid, X } from 'lucide-react';

const PhoneFrame: React.FC<{children: React.ReactNode}> = ({ children }) => (
  <div className="w-[320px] h-[640px] bg-white dark:bg-[#121212] rounded-[40px] border-[12px] border-black shadow-2xl overflow-hidden relative flex flex-col shrink-0">
    {children}
  </div>
);
...
export default function EcommerceShowcase() {
  ...
  const renderHome = () => (
    <div className="flex-1 overflow-y-auto scrollbar-hide bg-white dark:bg-[#121212] pb-24">
      {/* Header */}
      ...
      <div className="flex px-6 gap-6 mb-8 overflow-x-auto scrollbar-hide pb-2 animate-in fade-in slide-in-from-right-8 duration-500 delay-200">
        {['All', 'Furniture', 'Decor', 'Skincare', 'Lighting'].map((cat, i) => (
          <div key={i} className={`text-sm shrink-0 font-medium cursor-pointer transition-colors ${i === 0 ? 'text-black border-b-2 border-black dark:text-white dark:border-white pb-1' : 'text-gray-400 hover:text-gray-600'}`}>
            {cat}
          </div>
        ))}
      </div>
  ...
  const renderProductDetails = () => (
    <div className="absolute inset-0 bg-white dark:bg-[#121212] z-40 flex flex-col animate-in slide-in-from-bottom-full duration-500">
  ...
  const renderCart = () => (
    <div className="flex-1 flex flex-col bg-white dark:bg-[#121212] pb-20 animate-in fade-in duration-300">
      ...
      <div className="flex-1 overflow-y-auto px-6 pt-4">
        {cart.length === 0 ? (
          ...
          <div className="space-y-6">
            {cart.map((item, i) => (
              <div key={i} className="flex gap-4 animate-in slide-in-from-right-8 duration-500" style={{ animationDelay: `${i * 100}ms`, animationFillMode: 'both' }}>
      ...
      {cart.length > 0 && (
        <div className="p-6 border-t border-gray-100 dark:border-gray-800 bg-white dark:bg-[#121212] pt-4 mb-4">
  ...
  return (
    <div className="w-full h-full relative overflow-hidden bg-white dark:bg-[#121212] font-sans flex flex-col">
      {selectedProduct ? renderProductDetails() : (
         activeTab === 'home' ? renderHome() : renderCart()
      )}

      {/* Bottom Tab Bar */}
      {!selectedProduct && (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-black/90 dark:bg-white/90 backdrop-blur-md rounded-full px-6 py-3 flex items-center gap-8 z-30 shadow-2xl animate-in slide-in-from-bottom-12 duration-700 delay-500 fill-mode-both">
          <button 
            onClick={() => setActiveTab('home')}
          ...
          >
            ...
            {cart.length > 0 && (
              <span className="absolute -top-1.5 -right-1.5 bg-red-500 text-white text-[9px] w-3.5 h-3.5 rounded-full flex items-center justify-center font-bold border border-black">
                {cart.length}
              </span>
            )}
          </button>
