          ...
          children: [
            {
              ...
              type: "card",
              level: "component",
              props: { title: "Jane Doe", content: "Just finished a great hike in the mountains! \u{1F3D4}\uFE0F" },
              children: [
...
const generateId = (type) => `${type}_${Math.random().toString(36).substr(2, 5)}`;
const RegistryBuilderShowcase = () => {
  const [activeSchema, setActiveSchema] = useState(sampleLandingPage);
  const [selectedNodeId, setSelectedNodeId] = useState(sampleLandingPage.id);
  ...
  const deleteNode = (tree, id) => {
    ...
    if (tree.children) {
      const newChildren = tree.children.map((c) => deleteNode(c, id)).filter(Boolean);
  ...
  const handleAddNode = (registryId) => {
  ...
  const renderTree = (node, depth = 0) => {
    ...
    >
        ...
        {node.children && node.children.length > 0 && <div className={`${isDragOver ? "ml-2 border-l-2 border-blue-500 bg-blue-50/30 dark:bg-blue-900/10 rounded-b-md" : ""}`}>
             {node.children.map((child) => renderTree(child, depth + 1))}
           </div>}
      </div>;
  };
  return <div className="w-full h-screen flex flex-col bg-[#F2F2F7] dark:bg-black font-sans text-gray-900 dark:text-gray-100">
  ...
  }} className="shrink-0 px-3 py-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-white/5 text-sm font-medium transition-colors">Landing Page</button>
          <button onClick={() => {
    setActiveSchema(sampleMobileApp);
    setSelectedNodeId(sampleMobileApp.id);
  }} className="shrink-0 px-3 py-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-white/5 text-sm font-medium transition-colors">Mobile Dashboard</button>
          <button onClick={() => {
    setActiveSchema(sampleCommerceApp);
    setSelectedNodeId(sampleCommerceApp.id);
  }} className="shrink-0 px-3 py-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-white/5 text-sm font-medium transition-colors">Commerce</button>
          <button onClick={() => {
    setActiveSchema(sampleFinanceApp);
    setSelectedNodeId(sampleFinanceApp.id);
  }} className="shrink-0 px-3 py-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-white/5 text-sm font-medium transition-colors">Finance</button>
          <button onClick={() => {
    setActiveSchema(sampleFitnessApp);
    setSelectedNodeId(sampleFitnessApp.id);
  }} className="shrink-0 px-3 py-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-white/5 text-sm font-medium transition-colors">Fitness</button>
          <button onClick={() => {
    setActiveSchema(sampleSocialApp);
    setSelectedNodeId(sampleSocialApp.id);
  }} className="shrink-0 px-3 py-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-white/5 text-sm font-medium transition-colors">Social (Complex)</button>
          <button onClick={() => {
    setActiveSchema(sampleLearningApp);
    setSelectedNodeId(sampleLearningApp.id);
  }} className="shrink-0 px-3 py-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-white/5 text-sm font-medium transition-colors">Learning (Complex)</button>
          <button onClick={() => {
    setActiveSchema(invalidApp);
    setSelectedNodeId(invalidApp.id);
  }} className="shrink-0 px-3 py-1.5 rounded-md hover:bg-red-50 dark:hover:bg-red-900/20 text-red-600 text-sm font-medium transition-colors">Invalid Test</button>
        </div>
        <div className="flex items-center gap-2">
           {report?.valid ? <div className="flex items-center gap-1 text-sm font-medium text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20 px-3 py-1.5 rounded-full border border-green-200 dark:border-green-900">
                <CheckCircle className="w-4 h-4" /> Valid Schema
             </div> : <div className="flex items-center gap-1 text-sm font-medium text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-3 py-1.5 rounded-full border border-red-200 dark:border-red-900">
        ...
        <div className="w-72 bg-white dark:bg-[#1C1C1E] border-r border-gray-200 dark:border-[#38383A] flex flex-col shrink-0">
          ...
          <div className="flex-1 overflow-y-auto border-b border-gray-200 dark:border-[#38383A] p-3">
             ...
             <div className="space-y-4">
                {Object.entries(groupedRegistry).map(([level, items]) => <div key={level}>
  ...
  >
    ...
    /* Errors */
                 ...
                 {report && report.errors.some((e) => e.nodeId === selectedNode.id) && <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900 rounded-lg p-3 text-sm text-red-800 dark:text-red-400 space-y-2">
                     <div className="font-bold flex items-center gap-1"><AlertTriangle className="w-4 h-4" /> Validation Errors</div>
                     {report.errors.filter((e) => e.nodeId === selectedNode.id).map((err, i) => <div key={i} className="text-xs">
    ...
    /* Special case: Add explicit common props if not in default_props */
                     ...
                     {["title", "content", "label"].map((key) => selectedNode.props && typeof selectedNode.props[key] === "string" && (!selectedRegItem?.default_props || !(key in selectedRegItem.default_props)) ? <div key={"extra_" + key}>
  ...
  />
    ...
    /* Color Dropdown */
                     ...
                     {["button", "text", "badge"].includes(selectedNode.type) && <div>
