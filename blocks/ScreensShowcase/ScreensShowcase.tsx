...
import { IoChevronForwardOutline, IoMailOutline, IoLockClosedOutline, IoPersonOutline, IoArrowBackOutline, IoHomeOutline, IoMenuOutline, IoSearchOutline, IoHeartOutline, IoPersonCircleOutline, IoAppsOutline, IoDocumentOutline, IoExtensionPuzzleOutline, IoLayersOutline, IoAddOutline } from 'react-icons/io5';
...
export default function ScreensShowcase() {
  const [marketplaceCategory, setMarketplaceCategory] = useState<'All' | 'Apps' | 'Flows' | 'Screens' | 'Components'>('All');

  const createAppFromTemplate = (template: typeof APP_TEMPLATES[0]) => {
     try {
       ...
       Object.values(clonedScreens).forEach((screenBlocks: any) => {
           screenBlocks.forEach((block: any) => {
  ...
  const createFromTemplate = (template: typeof SCREEN_TEMPLATES[0]) => {
     try {
       const loaded = localStorage.getItem('ionic_builder_apps');
       const apps = loaded ? JSON.parse(loaded) : {};
       ...
       clonedBlocks.forEach((b: any) => b.id = `${b.id}-${Date.now()}-${Math.random()}`);
       
       currentApp[newName] = clonedBlocks;
       apps[appName] = currentApp;
       localStorage.setItem('ionic_builder_apps', JSON.stringify(apps));
       
       // Note: we can't tell the builder which active screen to show without some query param or advanced state
       // but we'll navigate anyway.
       window.location.href = '/page/Builder';
     } catch (e) { }
  };

  return (
    <div className="w-full max-w-7xl mx-auto h-[calc(100vh-64px)] p-4 md:p-8">
       <div className="w-full h-full bg-white dark:bg-[#1C1C1E] border border-gray-200 dark:border-gray-800 rounded-3xl shadow-xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-300 relative">
          ...
          <div className="flex bg-gray-50/50 dark:bg-[#151515] w-full overflow-x-auto border-b border-gray-200 dark:border-gray-800 shrink-0 custom-scrollbar px-6 md:px-10 space-x-2">
              {['All', 'Apps', 'Flows', 'Screens', 'Components'].map(cat => (
                  <button
                      key={cat}
                      onClick={() => setMarketplaceCategory(cat as any)}
          ...
          <div className="flex-1 overflow-y-auto p-6 md:p-10 custom-scrollbar">
            <div className="max-w-6xl mx-auto space-y-12 pb-20">
                {(marketplaceCategory === 'All' || marketplaceCategory === 'Apps') && (
                  <div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                          {APP_TEMPLATES.map(t => (
                              <button onClick={() => createAppFromTemplate(t)} key={t.id} className="group flex flex-col text-left bg-white dark:bg-[#1C1C1E] border border-gray-200 dark:border-gray-800 rounded-3xl overflow-hidden hover:border-indigo-400 hover:shadow-xl transition-all hover:-translate-y-1">
                ...
                {(marketplaceCategory === 'All' || marketplaceCategory === 'Screens') && (
                  <div>
                      <div className="flex items-center gap-3 mb-6">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-6">
                          {SCREEN_TEMPLATES.map(template => (
                              ...
                              >
                                  ...
                                  {template.id === 'blank' && (
                                      <div className="w-20 h-20 border-[3px] border-dashed border-gray-300 dark:border-gray-700 rounded-2xl flex items-center justify-center">
                                      <IoAddOutline className="w-10 h-10 text-gray-400 dark:text-gray-600" />
                                      </div>
                                  )}
                                  {template.id === 'app_shell' && (
                                      <div className="w-full h-full flex flex-col border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#1C1C1E] overflow-hidden rounded-xl shadow-sm">
                                  ...
                                  {template.id === 'list_view' && (
                                      <div className="w-full h-full flex flex-col border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#1C1C1E] overflow-hidden rounded-xl shadow-sm p-2">
                ...
                {(marketplaceCategory === 'Flows' || marketplaceCategory === 'Components') && (
